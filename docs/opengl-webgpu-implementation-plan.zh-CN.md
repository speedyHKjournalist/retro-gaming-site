...
| D-07 | 遮挡查询样本数 | WebGPU 的语义是“是否有样本通过”，可见时报告饱和的大样本数；跨 pass/提交的 GL 查询拆成 GPU 分段并对可见性取或，GL 查询结束且全部分段回读完成后才报告可用；丢弃旧代次，仅在回读失败时保守报告可见 | 用样本数做阈值判断的算法会得到饱和值，而非精确样本数；不再全局绕过遮挡查询 |

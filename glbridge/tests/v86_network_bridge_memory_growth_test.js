...
function warcraftStyleDrawElementsPayload(vertexBytes) {
    ...
    return Buffer.concat([
        ...
        clientArrayBlock(true, 2, GL_FLOAT, 8, Buffer.alloc(24, 0x31)),
        clientArrayBlock(true, 2, GL_FLOAT, 8, Buffer.alloc(24, 0x72)),
    ]);
}
...
function main() {
    ...
    const module = {
        ...
        _v86gl_glDrawElementsPackedMT(mode, count, indexType, indexPtr,
            ...
            const meta = new DataView(this.HEAPU8.buffer, metaPtr, 5 * 16);
    ...
    const payload = warcraftStyleDrawElementsPayload(Buffer.alloc(70000, 0x5A));
    ...
    assert.equal(freed.length, 5, "all packed array and metadata allocations must be freed");
